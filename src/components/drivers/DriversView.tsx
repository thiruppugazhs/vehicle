...
export const DriversView: React.FC = () => {
  ...
  return (
    <div className="space-y-6 text-left">
              ...
              <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-1">
                <div className="flex items-center gap-1">
                  ...
                  {assignedVeh && (
                    <button
                      type="button"
                      onClick={() => {
                        if (confirm(`Remove driver ${driver.name} from vehicle ${assignedVeh.registrationNumber}?`)) {
                          removeDriverAssignment(assignedVeh.id, assignedVeh.primaryDriverId === driver.id ? 'Primary' : 'Backup');
                          updateDriver(driver.id, { assignedVehicleId: undefined });
                        }
                      }}
                      className="inline-flex items-center gap-1 text-xs font-semibold text-slate-500 hover:text-rose-700 hover:bg-rose-50 px-2 py-1 rounded-lg transition-colors cursor-pointer"
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      if (confirm(`Delete driver profile for ${driver.name}?`)) deleteDriver(driver.id);
                    }}
      ...
      {/* Requirement 30: Edit Driver Modal */}
      {editingDriver && (
        <Modal
          isOpen={!!editingDriver}
          onClose={() => setEditingDriver(null)}
          title={`Edit Driver: ${editingDriver.name}`}
          subtitle="Update driver contact, license details, status, or emergency contact."
          maxWidth="md"
        >
          <form
            onSubmit={e => {
          ...
          >
            <div>
              ...
              <select
                ...
                onChange={e => {
                  const newStatus = e.target.value as 'Active' | 'On Leave' | 'Inactive';
                  updateDriver(editingDriver.id, { status: newStatus });
                  setEditingDriver(prev => prev ? { ...prev, status: newStatus } : null);
                }}
            ...
            <div>
              ...
              <input
                ...
                onChange={e => {
                  ...
                  updateDriver(editingDriver.id, { phone });
                  setEditingDriver(prev => prev ? { ...prev, phone } : null);
                }}
            ...
            <div>
              ...
              <input
                ...
                onChange={e => {
                  ...
                  updateDriver(editingDriver.id, { email });
                  setEditingDriver(prev => prev ? { ...prev, email } : null);
                }}
                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl"
              />
            </div>
            <div>
              <label className="block font-bold text-slate-700 mb-1">License Expiry</label>
              <input
                type="date"
                value={editingDriver.licenseExpiry}
                onChange={e => {
                  const licenseExpiry = e.target.value;
                  updateDriver(editingDriver.id, { licenseExpiry });
                  setEditingDriver(prev => prev ? { ...prev, licenseExpiry } : null);
                }}
                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={e => {
                  ...
                  updateDriver(editingDriver.id, { emergencyContact });
                  setEditingDriver(prev => prev ? { ...prev, emergencyContact } : null);
                }}
                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl"
              />
            </div>
